...
class Bird(pg.sprite.Sprite):
    ...
    def __init__(self, num: int, xy: tuple[int, int]):
        ...
        # 追加：無敵状態フラグ（Trueの間は被弾してもゲームオーバーにしない）
        self.invincible = False
        # 追加：無敵残りフレーム数（0以下で無敵終了）
        self.invincible_timer = 0

        # 追加：発射レート強化状態フラグ（Trueの間は発射レートを上げる）
        self.rapid_fire = False
        # 追加：発射レート（5 or 10）※update内でrapid_fireに応じて決定する
        self.shot_interval = 10
        # 追加：発射間隔管理用タイマー（shot_interval以上で発射可能）
        self.shot_timer = 0

    def skill(self, skill_count: int, fps: int = 50) -> tuple[bool, int]:
        """
        追加：スキル発動（無敵＋発射レート強化）
        引数1 skill_count：main側で管理している残りスキル回数
        引数2 fps：FPS（5秒をfps*5フレームに換算するために使用）
        戻り値：(発動できたか, 更新後skill_count)
        """
        if skill_count <= 0:
    ...
    def update(self, key_lst: list[bool], screen: pg.Surface):
        ...
        # 追加：無敵残り時間の更新（終了したら無敵・発射レート強化を解除）
        if self.invincible:
        ...
        # 追加：発射レートの決定（rapid_fireの状態で5/10を切り替える）
...
class SkillFlash(pg.sprite.Sprite):
    """
    追加：スキル発動時に画面全体を点滅させるフラッシュ演出
    引数1 life：存在フレーム数
    引数2 alpha_hi：明るい側の透明度
    引数3 alpha_lo：暗い側の透明度
    """
...
def main():
    ...
    while True:
        ...
        # 追加：Birdの発射レートに合わせて長押し射撃の間隔も同期する
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key == pg.K_q:
                # 追加：スキル発動（発動可否とskill_count更新を戻り値で行う）
